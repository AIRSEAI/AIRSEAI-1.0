...
from airseai_interface.srv import AirseaiGrasp, SegmentationGrasping
...
class GraspServer(Node):
    def __init__(self):
        ...
        self.declare_parameter('config', 'airseai_grasp_config.yaml')
        ...
        self.grasp_srv = self.create_service(
            AirseaiGrasp, '/airseai_grasp/grasp_server', self.grasp_callback, callback_group=MutuallyExclusiveCallbackGroup()
        )
        # Segmentation client
        self.seg_cli = self.create_client(
            SegmentationGrasping, '/airseai_perception/SegmentationGrasping', callback_group=MutuallyExclusiveCallbackGroup()
        )
        ...
        if np.any(mask):
            ...
            if not self.use_isaac_sim:
                ...
                if self.utility_elephant_robot.execute_grasp(grasp_coords):
                    response.ret = AirseaiGrasp.Response.SUCCESS
                    self.get_logger().info(f'Successfully grasped {obj}')
                else:
                    response.ret = AirseaiGrasp.Response.FAILED_TO_REACH_OBJECT
            ...
            response.ret = AirseaiGrasp.Response.FAILED_TO_FIND_OBJECT

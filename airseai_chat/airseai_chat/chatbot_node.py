...
from airseai_interface.srv import AirseaiInstruct
...
class voice_assistant(Node):
    ...
    def __init__(self):
        ...
        self.text_processor_client = self.create_client(AirseaiInstruct, '/airseai_planner/planner_server')
        if not self.text_processor_client.wait_for_service(timeout_sec=10.0):
            self.llm_server_ready = False
            return
        self.req = AirseaiInstruct.Request()
        ...
        try:
            ...
            for phrase in sphinx_speech:
                ...
                if self.llm_server_ready:
                    if str(phrase) in ["airseai", "air ship", "AIR SHIP", "AIRSEAI"]:

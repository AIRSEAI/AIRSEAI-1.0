...
from airseai_interface.srv import AirseaiNav
...
class NavigationService(Node):
    def __init__(self, node_name='navigation_service'):
        ...
        self.srv = self.create_service(AirseaiNav, '/airseai_navigation/navigate_to_pose', self.navigate_to_pose_callback)
    ...
    def navigate_to_pose_callback(self, request, response):
        ...
        if current_pose is None:
            ...
            response.status = AirseaiNav.Response.FAILED
            return response
        ...
        if not path:
            ...
            response.status = AirseaiNav.Response.FAILED_TO_FIND_PATH
            return response
        ...
        if result == TaskResult.SUCCEEDED:
            if goal_changed:
                ...
                response.status = AirseaiNav.Response.SUCCES_BY_REACHING_CLOEST_POINT
            else:
                self.get_logger().info('SUCCESS')
                response.status = AirseaiNav.Response.SUCCESS
        else:
            if timeout_occurred:
                self.get_logger().info('FAILED_TIMEOUT')
                response.status = AirseaiNav.Response.FAILED_TIMEOUT
            else:
